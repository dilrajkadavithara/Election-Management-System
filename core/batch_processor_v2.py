...
import logging
import re
from core.ocr_engine_v2 import OCREngine
from core.detector import VoterDetector

logger = logging.getLogger("BatchProcessorV2")

class BatchProcessor:
    def __init__(self):
        self.engine = OCREngine()
        logger.info(f"V2 Neural Sync {self.engine.VERSION} Ready.")
    ...
    def process_pdf_directly(self, pdf_path, batch_id=None, page_range=None, callback=None):
        ...
        V2: Uses thinking_budget=0 to eliminate ghost thinking tokens."""
        ...
        pages_set = set(pages)
        logger.info(f"V2 Pipeline Mode: {len(pages)} pages | {max_workers} workers | 300 DPI")

        temp_root = tempfile.mkdtemp(prefix="ocr_v2_batch_")
        page_to_image_map = {}
        # Cost tracking
        ...
        # SURGICAL RE-EXTRACTION disabled (same as V1)
        under_read_pages = []

        if under_read_pages and len(under_read_pages) <= 5:
            logger.info(f"V2 Surgical Phase: {len(under_read_pages)} pages identified as under-read: {[p['page'] for p in under_read_pages]}")

            # Re-render only the problem pages for surgical retry
            surgical_temp = tempfile.mkdtemp(prefix="ocr_v2_surgical_")
            try:
                surgical_images = {}
                for batch_paths in pdf_proc.convert_to_images(
                ...
                for page_info in under_read_pages:
                    ...
                    if not img_path:
                        logger.error(f"V2 Page {pg}: No image available for surgical retry")
                        continue

                    logger.info(f"V2 Page {pg}: {current_count} voters, missing {gap_count} (serials {gap_serials[0]}-{gap_serials[-1]}). Starting surgical extraction...")
                    ...
                    try:
                        ...
                        if success and isinstance(res, list) and len(res) > best_count:
                            logger.info(f"V2 Page {pg}: Tactic 1 (strong prompt) got {len(res)} voters (was {best_count})")
                            best_result = res
                            best_count = len(res)
                    except Exception as e:
                        logger.error(f"V2 Page {pg}: Tactic 1 failed: {e}")

                    # Tactic 2: If still under-read, split into 3 column images
                    if best_count < current_count + gap_count - 2:  # Allow 2 margin
                        try:
                            logger.info(f"V2 Page {pg}: Tactic 2 - Column split extraction...")
                            column_results = self.engine.extract_column_split(img_path, page_num=pg)
                            if column_results and len(column_results) > best_count:
                                logger.info(f"V2 Page {pg}: Column split got {len(column_results)} voters (was {best_count})")
                                best_result = column_results
                                best_count = len(column_results)
                        except Exception as e:
                            logger.error(f"V2 Page {pg}: Tactic 2 failed: {e}")

                    # Tactic 3: If STILL under-read, retry with temperature=0.3
                    if best_count < current_count + gap_count - 2:
                        try:
                            logger.info(f"V2 Page {pg}: Tactic 3 - Temperature variation...")
                            p_num, res, success = self.engine.extract_full_page_consolidated(
                                img_path, page_num=pg, force_full_scan=True, temperature=0.3
                            )
                            if success and isinstance(res, list) and len(res) > best_count:
                                logger.info(f"V2 Page {pg}: Temperature variation got {len(res)} voters (was {best_count})")
                                best_result = res
                                best_count = len(res)
                        except Exception as e:
                            logger.error(f"V2 Page {pg}: Tactic 3 failed: {e}")

                    # Apply best result
                    if best_count > current_count:
                        ordered_results[pg] = best_result
                        logger.info(f"V2 Page {pg}: Surgical success! {current_count} → {best_count} voters")
                        if callback:
                            callback(pg, best_result, True)
                    else:
                        logger.warning(f"V2 Page {pg}: Surgical extraction did not improve. Keeping {current_count} voters.")
        ...
        if all_serials:
            ...
            if still_missing:
                logger.warning(f"V2 Final validation: {len(still_missing)} serial gaps remain: {sorted(still_missing)[:20]}...")
            else:
                logger.info(f"V2 Final validation: All {len(all_serials)} serials are continuous. Zero gaps!")
        ...
        logger.info(f"V2 Cost Report: {total_cost['api_calls']} API calls | "
    ...
    def _analyze_serial_gaps(self, ordered_results, pages):
        ...
        if not all_missing:
            logger.info(f"V2 Gap Analysis: All serials {min_serial}-{max_serial} present. No under-read pages.")
            return []

        logger.info(f"V2 Gap Analysis: {len(all_missing)} missing serials detected in range {min_serial}-{max_serial}")

        # Check ALL pages for serial gaps
        under_read_pages = []
        ...
        for i, p in enumerate(sorted_pages):
            ...
            if page_missing:
                ...
                logger.info(f"V2 Page {p}: {len(results)} voters found, {len(page_missing)} missing (serials {missing_sorted[0]}-{missing_sorted[-1]}). UNDER-READ.")
            else:
                logger.info(f"V2 Page {p}: {len(results)} voters — no serial gaps. GENUINELY PARTIAL.")
