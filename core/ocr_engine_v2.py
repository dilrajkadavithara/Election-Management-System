...
logger = logging.getLogger("OCREngineV2")

class OCREngine:
    VERSION = "5.0.0-V2-OPTIMIZED"
    ...
    def extract_full_page_consolidated(self, img, page_num="?", force_full_scan=False, temperature=0.0):
        ...
        logger.info(f"V2 Neural Sync: Page {page_num} -> Initiating API Request...{' (FULL SCAN MODE)' if force_full_scan else ''}{f' (temp={temperature})' if temperature > 0 else ''}")
        ...
        if results:
            logger.info(f"V2 Neural Sync: Page {page_num} -> Extraction Successful ({len(results)} voters found)")
            return page_num, results, True

        logger.error(f"V2 Neural Sync: Page {page_num} -> Final failure after retries.")
    ...
    def extract_column_split(self, img_path, page_num="?"):
        ...
        logger.info(f"V2 Column Split: Page {page_num} — splitting into 3 columns")
            ...
            for col_idx, col_img in enumerate(columns):
                ...
                try:
                    ...
                    if results and isinstance(results, list):
                        logger.info(f"V2 Column Split: Page {page_num} {col_label} → {len(results)} voters")
                        ...
                        for voter in results:
                            ...
                            if serial and serial not in seen_serials:
                                ...
                                logger.debug(f"V2 Column Split: Duplicate serial {serial} skipped")
                except Exception as e:
                    logger.error(f"V2 Column Split: Page {page_num} {col_label} failed: {e}")

            # Sort by serial number
            all_voters.sort(key=lambda v: int(v.get("Serial_OCR", 0)) if str(v.get("Serial_OCR", "")).isdigit() else 0)
            logger.info(f"V2 Column Split: Page {page_num} total after merge: {len(all_voters)} unique voters")
            return all_voters if all_voters else None

        except Exception as e:
            logger.error(f"V2 Column Split: Page {page_num} failed entirely: {e}")
            return None

    def detect_voter_boxes(self, img_path):
        """Use OpenCV to detect the grid of voter boxes on a page.

        Kerala voter lists have a consistent 3-column x 10-row grid.
...
Maintain 100% literal accuracy for all Malayalam text."""
    ...
    def extract_with_box_guidance(self, img_path, page_num="?"):
        ...
        logger.info(f"V2 Neural Sync: Page {page_num} -> OpenCV detected {box_count} boxes. Cropping for 5-box batch extraction...")
        ...
        logger.info(f"V2 Neural Sync: Page {page_num} -> {len(batches)} batches of ~{BATCH_SIZE} boxes")
        ...
        logger.info(f"V2 Neural Sync: Page {page_num} -> Per-box extraction: {len(valid_results)} voters from {box_count} boxes ({total_tokens['api_calls']} calls, {total_tokens['input']}+{total_tokens['output']} tokens)")
