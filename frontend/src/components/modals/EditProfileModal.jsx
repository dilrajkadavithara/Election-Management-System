...
const EditProfileModal = ({
    ...
    return (
        <div
            className="fixed top-0 right-0 bottom-0 left-0 lg:left-[420px] bg-slate-900/60 backdrop-blur-md z-[1100] flex items-center justify-center p-2 sm:p-4 lg:pl-4"
            onClick={(e) => { if (e.target === e.currentTarget) closeModal(); }}
        >
            <div className="bg-white w-full max-w-2xl rounded-3xl sm:rounded-[3.5rem] shadow-[0_40px_120px_rgba(0,0,0,0.4)] overflow-hidden relative animate-in fade-in slide-in-from-bottom-8 duration-500 max-h-[92vh] sm:max-h-[95vh] flex flex-col">

                {/* Sticky top bar — always visible close + save on mobile */}
                <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm border-b border-slate-100 px-4 py-3 sm:px-10 sm:py-4 flex items-center justify-between flex-shrink-0">
                    <button
                        onClick={closeModal}
                        className="w-9 h-9 sm:w-10 sm:h-10 bg-slate-100 hover:bg-slate-200 rounded-full flex items-center justify-center text-slate-500 hover:text-slate-700 transition-all active:scale-90"
                        title="Close"
                    >
                    ...
                    <span className="text-[9px] sm:text-[10px] font-black text-slate-400 tracking-[0.15em] hidden sm:block">വിവര ശേഖരണം</span>
                    <div className="flex items-center gap-2">
                        {ocrBatch && (
                            <button
                                onClick={deleteRecord}
                                className="w-9 h-9 sm:w-10 sm:h-10 bg-rose-50 hover:bg-rose-600 rounded-full flex items-center justify-center text-sm hover:text-white transition-all border border-rose-100"
                                title="Delete"
                            >
                                🗑️
                            </button>
                        ...
                        <button
                            ...
                            className="lg:hidden bg-indigo-600 text-white px-4 py-2 rounded-xl text-[10px] font-black tracking-wider hover:bg-indigo-700 active:scale-95 transition-all"
                        >
                            Save
                        </button>
                    </div>
                ...
                {/* Scrollable content */}
                <div className="overflow-y-auto flex-1 p-5 sm:p-14 space-y-6 sm:space-y-12 custom-scrollbar">
                    ...
                    <div className="text-center space-y-1 sm:space-y-3 relative">
                        ...
                        <div className="flex justify-center flex-wrap gap-2 pt-1 sm:pt-2">
                            <span className="text-[8px] sm:text-[9px] font-black bg-slate-100 text-slate-500 px-3 py-1 sm:px-5 sm:py-2 rounded-full tracking-tighter shadow-sm border border-slate-200/50">
                                {editData.gender === 'MALE' ? 'MALE' : editData.gender === 'FEMALE' ? 'FEMALE' : editData.gender || '??'}
                            </span>
                            <span className="text-[8px] sm:text-[9px] font-black bg-slate-100 text-slate-500 px-3 py-1 sm:px-5 sm:py-2 rounded-full tracking-tighter shadow-sm border border-slate-200/50">
                                {editData.age || 'UNK'} yrs
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4 sm:space-y-12">
                        ...
                        <details className="group border-b border-slate-100 pb-8">
                            <summary className="text-[10px] sm:text-[11px] font-black text-indigo-500 cursor-pointer list-none flex items-center gap-3">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6 animate-in fade-in slide-in-from-top-2">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="space-y-1">
                                        <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">House No</label>
                                        <input
                                            type="text"
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">House Name</label>
                                        <input
                                            type="text"
                        ...
                        <div className="space-y-2 sm:space-y-4">
                            <label className="text-[10px] sm:text-[11px] font-black text-slate-400 tracking-[0.15em] ml-1 sm:ml-2">ഗൃഹനാഥനാണോ (Head of Family)?</label>
                            <button
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className={`text-sm sm:text-lg font-black tracking-wider ${editData.is_head_of_family ? 'text-amber-700' : 'text-slate-400'}`}>
                                        {editData.is_head_of_family ? 'YES' : 'NO'}
                                    </span>
                                </div>
                    ...
                    {/* Submit & Cancel — desktop only (mobile has sticky top bar) */}
                    <div className="hidden sm:flex flex-row gap-2 sm:gap-5 pt-0 sm:pt-4">
                        <button
                            onClick={closeModal}
