    ...
    return (
        <div className="v2-glass p-8 rounded-[2.5rem] relative">
                ...
                {boothStats.map((booth, index) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {/* Tooltip Content Removed from here, moved to Portal below */}
                        </button>
                    );
                })}
            </div>
            ...
            {hoverInfo && createPortal(
                <div
                    className="fixed z-[9999] w-64 pointer-events-none animate-in fade-in zoom-in duration-200"
                    style={{
                        left: hoverInfo.x + (hoverInfo.width / 2),
                        top: hoverInfo.showBelow ? hoverInfo.y + hoverInfo.height + 8 : hoverInfo.y - 8,
                        transform: `translateX(-50%) ${hoverInfo.showBelow ? '' : 'translateY(-100%)'}`
                    }}
                >
                    <div className="bg-white p-5 rounded-[1.5rem] text-[11px] shadow-[0_30px_60px_rgba(0,0,0,0.6)] border border-slate-200 pointer-events-auto">
                        <div className="flex justify-between items-center border-b border-slate-100 pb-3 mb-3">
                            <span className="font-black text-slate-900 uppercase text-xs">Booth {hoverInfo.number}</span>
                            <span className="px-2.5 py-1 bg-blue-50 text-blue-600 rounded-full text-[9px] font-black uppercase tracking-tighter">
                                Coverage: {hoverInfo.coverage}%
                            </span>
                        </div>
                        <div className="space-y-2.5 text-slate-600">
                            <div className="flex justify-between font-bold border-b border-slate-50 pb-1"><span>Total Voters</span> <span className="text-slate-900">{Number(hoverInfo.total) || 0}</span></div>
                            <div className="flex justify-between items-center text-blue-600 font-bold">
                                <span>UDF Supporters</span>
                                <span className="font-black text-lg leading-none">{Number(hoverInfo.udf) || 0}</span>
                            </div>
                            <div className="flex justify-between items-center text-rose-600 font-bold">
                                <span>LDF Supporters</span>
                                <span className="font-black text-lg leading-none">{Number(hoverInfo.ldf) || 0}</span>
                            </div>
                            <div className="flex justify-between items-center text-orange-600 font-bold">
                                <span>NDA Supporters</span>
                                <span className="font-black text-lg leading-none">{Number(hoverInfo.nda) || 0}</span>
                            </div>
                            <div className="flex justify-between items-center text-slate-500 font-bold">
                                <span>Neutral Voters</span>
                                <span className="font-black text-slate-700">{Number(hoverInfo.neutral) || 0}</span>
                            </div>
                            <div className="flex justify-between text-slate-400 border-t border-slate-50 pt-2 mt-1 italic">
                                <span>Remaining Data Gap</span>
                                <span className="font-bold text-slate-500">{(Number(hoverInfo.total) || 0) - (Number(hoverInfo.udf) || 0) - (Number(hoverInfo.ldf) || 0) - (Number(hoverInfo.nda) || 0) - (Number(hoverInfo.neutral) || 0)}</span>
                            </div>
                        </div>
                        <div className="mt-4 text-center text-[9px] text-blue-600 font-black uppercase tracking-widest bg-blue-50 py-3 rounded-xl border border-blue-100 shadow-sm">
                            Click Square to Open List
                        </div>
                    </div>
                </div>,
                document.body
