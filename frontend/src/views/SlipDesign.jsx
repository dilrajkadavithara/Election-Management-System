...
const SlipDesign = ({
    ...
    useEffect(() => {
        ...
        setTimeout(() => loadVoters(1), 100);
    }, []);

    const handleReset = () => {
        setListFilters({ constituency: '', lb: '', booth: '', gender: '', ageFrom: '', ageTo: '', leaning: '', serialFrom: '', serialTo: '', location: '' });
        setTimeout(() => loadVoters(1), 100);
    };

    return (
        <div className="min-h-screen lux-mesh-bg p-12 pl-[420px] pr-16 space-y-16 lux-animate-in pb-32">
            <header className="flex flex-wrap justify-between items-end border-b border-white/5 pb-10 no-print gap-12">
                <div>
                    <h1 className="text-7xl font-black tracking-tighter uppercase lux-text-gradient">Voter Slips</h1>
                    <p className="text-slate-300 font-bold uppercase tracking-widest text-sm mt-3 ml-1">Generate and Print Slips for Distribution</p>
                </div>
                <div className="flex gap-6 items-center">
                    <button
                        onClick={() => window.print()}
                        disabled={voterList.length === 0}
                        className="lux-btn-primary !py-6 !px-10 text-sm tracking-widest shadow-[0_0_40px_rgba(99,102,241,0.3)] group overflow-hidden relative active:scale-95"
                    >
            ...
            <div className="lux-glass p-8 rounded-[3rem] border-white/5 shadow-2xl space-y-6 no-print group">
                <div className="grid grid-cols-2 lg:grid-cols-5 gap-6 items-end">
                    ...
                    <div className="col-span-2 lg:col-span-3 flex gap-4 mt-2 h-11">
                        <button onClick={() => loadVoters(1)} className="lux-btn-primary flex-1 !p-0 max-w-[200px] text-xs tracking-widest shadow-xl">APPLY FILTERS</button>
                        <button onClick={handleReset} className="bg-white/5 text-slate-300 flex-1 max-w-[150px] rounded-xl text-xs font-black uppercase tracking-widest hover:bg-white/10 transition-all">CLEAR ALL</button>
                    </div>
                </div>
            </div>

            <div className="flex justify-center no-print pb-4">
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Showing Preview of Slips Generated Using Applied Filters</p>
            </div>
